...
def read_images(path):
    return [{'path': os.path.join(path, filename), 'img': cv2.imread(os.path.join(path, filename))} for filename in os.listdir(path)]
...
def process_image(dataset, grid_size, cell_size):
    return [{'path': img['path'], 'descriptors': calculate_histograms(img['img'], grid_size, cell_size)} for img in dataset]

def process_search_images(dataset_search, dataset_index, grid_size, cell_size):
    return [{'search_path': img['path'], 'search_descriptors': calculate_histograms(img['img'], grid_size, cell_size), 'index': dataset_index} for img in dataset_search]
